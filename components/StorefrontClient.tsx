...
export default function StorefrontClient({
  ...
  async function sendToWhatsApp() {
    ...
    if (orderError || !order) {
      setSubmitting(false);
      setSubmitError("Couldn't save your order. Please try again.");
      return;
    }

    // 2. Save each cart item against that order
    ...
    if (itemsError) {
      // The order itself saved fine — WhatsApp checkout can still proceed,
      // this just means the line-item breakdown won't show in the dashboard.
          ...
          <div className="bg-white w-full max-w-sm h-full p-6 overflow-y-auto">
            ...
            {cart.length === 0 ? (
              ...
              <>
                ...
                {submitError && (
                  <div className="mb-3 text-xs bg-[#f6e2e2] text-[#c14545] rounded-lg px-3 py-2">
                    {submitError}
                  </div>
                )}
                ...
                <button
                  ...
                  disabled={submitting}
                  className="w-full bg-[#25D366] text-white font-bold py-3 rounded-lg disabled:opacity-60"
                >
